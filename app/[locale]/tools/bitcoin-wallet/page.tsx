...
const toolSlug = 'bitcoin-wallet';

type BitcoinWalletPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: BitcoinWalletPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/bitcoin-wallet'),
    keywords: tool
...
export default async function BitcoinWalletPage({ params }: BitcoinWalletPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
