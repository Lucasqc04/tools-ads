...
import { UuidNanoIdGeneratorTool } from '@/components/tools/uuid-nanoid-generator-tool';
...
const toolSlug = 'gerador-de-uuid-e-nanoid';

type UuidNanoIdPageProps = Readonly<{
  params: Promise<{ locale: string }>;
}>;

export async function generateMetadata({ params }: UuidNanoIdPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/gerador-de-uuid-e-nanoid'),
    keywords: tool ? [tool.primaryKeyword, ...tool.secondaryKeywords] : dictionary.seo.tools.keywords,
  });
}

export default async function UuidNanoIdPage({ params }: UuidNanoIdPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<UuidNanoIdGeneratorTool locale={locale} />}
      />
    </>
  );
}
