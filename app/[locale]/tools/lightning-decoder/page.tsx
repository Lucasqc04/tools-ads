...
import { LightningDecoderTool } from '@/components/tools/lightning-decoder-tool';
...
const toolSlug = 'lightning-decoder';

type LightningDecoderPageProps = {
  params: Promise<{
    locale: string;
  }>;
};
...
export async function generateMetadata({
  params,
}: LightningDecoderPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/lightning-decoder'),
    keywords: tool
...
export default async function LightningDecoderPage({
  params,
}: Readonly<LightningDecoderPageProps>) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<LightningDecoderTool locale={locale} />}
      />
    </>
  );
}
