...
import { ImageColorExtractorTool } from '@/components/tools/image-color-extractor-tool';
...
const toolSlug = 'image-color-extractor';
...
export async function generateMetadata({
  params,
}: ImageColorExtractorPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/image-color-extractor'),
    keywords: tool
...
export default async function ImageColorExtractorPage({ params }: Readonly<ImageColorExtractorPageProps>) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<ImageColorExtractorTool locale={locale} />}
      />
    </>
  );
}
