...
const toolSlug = 'keyboard-shortcuts';

type KeyboardShortcutsPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: KeyboardShortcutsPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/keyboard-shortcuts'),
    keywords: tool
...
export default async function KeyboardShortcutsPage({ params }: KeyboardShortcutsPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<KeyboardShortcutsTool locale={locale} />}
      />
    </>
  );
}
