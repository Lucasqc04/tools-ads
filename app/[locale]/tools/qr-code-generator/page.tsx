...
export async function generateMetadata({
  params,
}: QrCodeGeneratorPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    locale,
    title: tool?.seoTitle ?? dictionary.seo.qrCodeFallback.title,
    description: tool?.seoDescription ?? dictionary.seo.qrCodeFallback.description,
    localePaths: buildLocalePathMap('/tools/qr-code-generator'),
    keywords: tool
      ? [tool.primaryKeyword, ...tool.secondaryKeywords]
      : dictionary.seo.qrCodeFallback.keywords,
  });
}

export default async function QrCodeGeneratorPage({ params }: QrCodeGeneratorPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
