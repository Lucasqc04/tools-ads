...
import { CssGeneratorTool } from '@/components/tools/css-generator-tool';
...
const toolSlug = 'css-generator';

type CssGeneratorPageProps = {
  params: Promise<{
    locale: string;
  }>;
};
...
export async function generateMetadata({
  params,
}: CssGeneratorPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/css-generator'),
    keywords: tool
...
export default async function CssGeneratorPage({ params }: Readonly<CssGeneratorPageProps>) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<CssGeneratorTool locale={locale} />}
      />
    </>
  );
}
