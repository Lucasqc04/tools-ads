...
import { RemoveAccentsTool } from '@/components/tools/remove-accents-tool';
...
const toolSlug = 'remover-acentos';

type RemoveAccentsPageProps = Readonly<{
  params: Promise<{ locale: string }>;
}>;

export async function generateMetadata({ params }: RemoveAccentsPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/remover-acentos'),
    keywords: tool ? [tool.primaryKeyword, ...tool.secondaryKeywords] : dictionary.seo.tools.keywords,
  });
}

export default async function RemoveAccentsPage({ params }: RemoveAccentsPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<RemoveAccentsTool locale={locale} />}
      />
    </>
  );
}
