...
const toolSlug = 'symbols-to-copy';

type SymbolsToCopyPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: SymbolsToCopyPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/symbols-to-copy'),
    keywords: tool
...
export default async function SymbolsToCopyPage({ params }: SymbolsToCopyPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<SymbolsToCopyTool locale={locale} />}
      />
    </>
  );
}
