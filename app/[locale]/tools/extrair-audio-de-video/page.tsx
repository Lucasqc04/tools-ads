...
const toolSlug = 'extrair-audio-de-video';

type AudioEditorPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: AudioEditorPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/extrair-audio-de-video'),
    keywords: tool
...
export default async function AudioEditorPage({ params }: AudioEditorPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<AudioEditorTool locale={locale} />}
      />
    </>
  );
}
