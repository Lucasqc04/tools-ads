...
const toolSlug = 'color-converter';

type ColorConverterPageProps = {
  params: Promise<{
    locale: string;
  }>;
};
...
export async function generateMetadata({
  params,
}: ColorConverterPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/color-converter'),
    keywords: tool
...
export default async function ColorConverterPage({ params }: Readonly<ColorConverterPageProps>) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<ColorConverterTool locale={locale} />}
      />
    </>
  );
}
