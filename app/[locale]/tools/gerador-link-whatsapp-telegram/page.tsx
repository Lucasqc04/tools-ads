...
const toolSlug = 'gerador-link-whatsapp-telegram';

type WhatsAppTelegramLinkGeneratorPageProps = Readonly<{
  params: Promise<{ locale: string }>;
}>;

export async function generateMetadata({
  params,
}: WhatsAppTelegramLinkGeneratorPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/gerador-link-whatsapp-telegram'),
    keywords: tool ? [tool.primaryKeyword, ...tool.secondaryKeywords] : dictionary.seo.tools.keywords,
  });
}

export default async function WhatsAppTelegramLinkGeneratorPage({
  params,
}: WhatsAppTelegramLinkGeneratorPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<WhatsAppTelegramLinkGeneratorTool locale={locale} />}
      />
    </>
  );
}
