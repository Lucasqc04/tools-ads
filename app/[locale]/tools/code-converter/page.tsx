...
import { CodeConverterTool } from '@/components/tools/code-converter-tool';
...
const toolSlug = 'code-converter';

type CodeConverterPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: CodeConverterPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/code-converter'),
    keywords: tool
...
export default async function CodeConverterPage({ params }: CodeConverterPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<CodeConverterTool locale={locale} />}
      />
    </>
  );
}
