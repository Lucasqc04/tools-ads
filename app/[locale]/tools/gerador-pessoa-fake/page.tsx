...
const toolSlug = 'gerador-pessoa-fake';

type FakePersonGeneratorPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: FakePersonGeneratorPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/gerador-pessoa-fake'),
    keywords: tool
...
export default async function FakePersonGeneratorPage({ params }: FakePersonGeneratorPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
