...
import { IpDiscoveryTool } from '@/components/tools/ip-discovery-tool';
...
const toolSlug = 'descobrir-ip-publico';

type IpDiscoveryPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;

export async function generateMetadata({ params }: IpDiscoveryPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/descobrir-ip-publico'),
    keywords: tool ? [tool.primaryKeyword, ...tool.secondaryKeywords] : dictionary.seo.tools.keywords,
  });
}

export default async function IpDiscoveryPage({ params }: IpDiscoveryPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<IpDiscoveryTool locale={locale} />}
      />
    </>
  );
}
