...
const toolSlug = 'authenticator-code-generator';

type AuthenticatorCodeGeneratorPageProps = Readonly<{
  params: Promise<{ locale: string }>;
}>;

export async function generateMetadata({
  params,
}: AuthenticatorCodeGeneratorPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/authenticator-code-generator'),
    keywords: tool ? [tool.primaryKeyword, ...tool.secondaryKeywords] : dictionary.seo.tools.keywords,
  });
}

export default async function AuthenticatorCodeGeneratorPage({
  params,
}: AuthenticatorCodeGeneratorPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<AuthenticatorCodeGeneratorTool locale={locale} />}
      />
    </>
  );
}
