...
import { AudioVideoTranscriptionTool } from '@/components/tools/audio-video-transcription-tool';
...
const toolSlug = 'transcricao-de-audio-e-video';

type AudioVideoTranscriptionPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: AudioVideoTranscriptionPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/transcricao-de-audio-e-video'),
    keywords: tool
...
export default async function AudioVideoTranscriptionPage({
  params,
}: AudioVideoTranscriptionPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<AudioVideoTranscriptionTool locale={locale} />}
      />
    </>
  );
}
