...
const toolSlug = 'calculadora-juros-compostos';

type CompoundInterestPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: CompoundInterestPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/calculadora-juros-compostos'),
    keywords: tool
...
export default async function CompoundInterestPage({ params }: CompoundInterestPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
