...
const toolSlug = 'pix-decoder';

type PixDecoderPageProps = {
  params: Promise<{
    locale: string;
  }>;
};
...
export async function generateMetadata({
  params,
}: PixDecoderPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/pix-decoder'),
    keywords: tool
...
export default async function PixDecoderPage({ params }: Readonly<PixDecoderPageProps>) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<PixDecoderTool locale={locale} />}
      />
    </>
  );
}
