...
const toolSlug = 'multiplication-table-quiz';

type MultiplicationTableQuizPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: MultiplicationTableQuizPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/multiplication-table-quiz'),
    keywords: tool
...
export default async function MultiplicationTableQuizPage({
  params,
}: MultiplicationTableQuizPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
  ...
  return (
    <>
      ...
      <ToolPageShell
        ...
        toolUi={<MultiplicationTableQuizTool locale={locale} />}
      />
    </>
  );
}
