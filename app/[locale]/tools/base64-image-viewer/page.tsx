...
const toolSlug = 'base64-image-viewer';

type Base64ImageViewerPageProps = Readonly<{
  params: Promise<{
    locale: string;
  }>;
}>;
...
export async function generateMetadata({
  params,
}: Base64ImageViewerPageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({
    ...
    localePaths: buildLocalePathMap('/tools/base64-image-viewer'),
    keywords: tool
...
export default async function Base64ImageViewerPage({ params }: Base64ImageViewerPageProps) {
  const { locale: localeParam } = await params;
  const locale = resolveLocale(localeParam);
  const dictionary = getDictionary(locale);
  const tool = getLocalizedToolBySlug(locale, toolSlug);
