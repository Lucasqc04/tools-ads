import Script from 'next/script';

type GoogleTagManagerProps = Readonly<{
  containerId?: string;
}>;

const isValidContainerId = (value: string | undefined): value is string =>
  Boolean(value && /^GTM-[A-Z0-9]+$/i.test(value));

/** Loads GTM only when a valid public container id is configured. */
export function GoogleTagManager({ containerId }: GoogleTagManagerProps) {
  if (!isValidContainerId(containerId)) {
    return null;
  }

  return (
    <>
      <Script id="google-tag-manager" strategy="afterInteractive">
        {`(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','${containerId}');`}
      </Script>
      <noscript>
        <iframe
          src={`https://www.googletagmanager.com/ns.html?id=${containerId}`}
          height="0"
          width="0"
          style={{ display: 'none', visibility: 'hidden' }}
        />
      </noscript>
    </>
  );
}
